...
const bars = computed(() => activeRange.value === 'week'
  ? [42, 68, 53, 86, 61, 76, 48]
  : [56, 63, 48, 72, 66, 83, 70])
  ...
  <div class="profile-page">
    ...
    <div class="content-grid">
      <section class="surface activity-card">
        <div class="card-heading"><div><h2>学习活跃度</h2><p>有效学习时长分布</p></div><div class="segmented"><button :class="{ active: activeRange === 'week' }" @click="activeRange = 'week'">本周</button><button :class="{ active: activeRange === 'month' }" @click="activeRange = 'month'">本月</button></div></div>
        <div class="chart">
          <div v-for="(height, index) in bars" :key="index" class="bar-column"><span>{{ [2.1, 3.4, 2.7, 4.3, 3.1, 3.8, 2.4][index] }}h</span><i :style="{ height: `${height}%` }" /><small>{{ ['一','二','三','四','五','六','日'][index] }}</small></div>
        </div>
      </section>
      ...
      <section class="surface memory-card">
        <div class="memory-orb">✦</div><div><p class="eyebrow">FLOW AGENT · 反馈记忆</p><h2>已读取 {{ memories.items.length }} 条有效记忆</h2><p>{{ memories.pendingCount ? `其中 ${memories.pendingCount} 条待你确认，尚不会影响 Agent。` : '当前没有待确认记忆。' }}</p><button class="text-button" @click="openMemories">查看与管理记忆 →</button></div>
      </section>
      ...
      <section class="surface plan-card">
        ...
        <button class="primary-button" @click="notify('已打开本周计划记录')">查看计划记录</button>
      </section>
    </div>
